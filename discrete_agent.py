...
class DiscreteAgent(Agent):
    """
    Create an Agent that uses a discrete representation of the state space
      to implement algorithms for limited state space size (such as QLearning)
    ...
    def convert_state_to_discrete_space(self, state):
        ...
        discrete_state = [
            round(
                (pos0 + self.max_space0) / (2 * self.N_space_grid * self.max_space0)
            ),  # Negative and positive pos0 so 2*max_space0 chunk size
        ...
        if (np.array(discrete_state) < 0).any() or (
            np.array(discrete_state)
            > np.array(
                [self.N_space_grid, self.N_space_grid, self.max_theta, self.max_theta_dot, self.max_vel0, self.max_vel1]
            )
        ).any():
...
class QLearning_Agent(DiscreteAgent):
    """Implement a QLearning Agent using a discrete representation
      of the state space given by the DiscreteAgent framework
    ...
    def train(
        self,
        env=ParachutistEnv(pygame_used=False),
        alpha=0.1,
        alpha_factor=0.9995,
        gamma=0.99,
        num_episodes=1000,
